...
local sendCommandSignals = function(self)
	for _, commandEvent in self._inputDevice:pollEvents() do
		if self._disabled > 0 then return; end
...
InputDrivenController.getAssignedPlayer = function(self) return self._playerIndex; end

InputDrivenController.getInputDevice = function(self) return self._inputDevice; end
...
InputDrivenController.waitForCommandPress = function(self, command)
	if self._inputDevice:isCommandActive(command) then self:waitFor("-" .. command); end
	self:waitFor("+" .. command);
end

InputDrivenController.isCommandActive = function(self, command)
	if self._disabled > 0 then return false; end
	return self._inputDevice:isCommandActive(command);
end

InputDrivenController.disable = function(self) self._disabled = self._disabled + 1; end

InputDrivenController.enable = function(self) self._disabled = self._disabled - 1; end
