...
local setDevBotBehavior = function(behavior) DevBotController._behavior = behavior; end

CLI:addCommand("setDevBotBehavior behavior:string", setDevBotBehavior);

-- PUBLIC API

DevBotController.init = function(self, entity) DevBotController.super.init(self, entity, self.run); end
...
DevBotController.run = function(self)
	...
	while true do
		if self:isIdle() then
			if DevBotController._behavior == "idle" then self:doAction(Actions.idle); end
			if DevBotController._behavior == "walk" then self:doAction(Actions.walk(entity:getAngle())); end
			...
			if DevBotController._behavior == "attack" then self:doAction(Actions.attack); end
		end
		if self:isTaskless() then
			if DevBotController._behavior == "walkToPoint" then self:doTask(Movement.walkToPoint(32, 200, 6)); end
