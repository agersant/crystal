...
local LineScanner = require("external.luacov.linescanner")
local luacov = require("external.luacov.runner")
local util = require("external.luacov.util")
...
do
	...
	function ReporterBase:new(conf)
		local stats = require("external.luacov.stats")
		...
		local o = setmetatable({_out = out, _cfg = conf, _data = filtered_data, _files = files, _mhit = max_hits}, self)
	...
	function DefaultReporter:on_end()
		...
		for _, filename in ipairs(self:files()) do
			...
			if summary then
				...
				table.insert(lines, {filename, tostring(summary.hits), tostring(summary.miss), coverage_to_string(hits, missed)})
		...
		table.insert(lines,
             		{"Total", tostring(total_hits), tostring(total_missed), coverage_to_string(total_hits, total_missed)})
