...
static int isPointInsideNavmeshTriangle(const BNavmesh* navmesh, const BVector* point,
										const BTriangle* triangle) {
...
static const BTriangle* findTriangleContainingPoint(const BNavmesh* navmesh, const BVector* point) {
	...
	// TODO.optimization This could be more efficient if triangles were stored
	// in a more appropriate data structure (eg. quadtree).
...
static void projectPointOntoNavmeshTriangle(const BNavmesh* navmesh, const BVector* point,
											const BTriangle* triangle, BVector* outPoint) {
	const BVector* vertexA = &navmesh->vertices[triangle->vertices[0]];
	const BVector* vertexB = &navmesh->vertices[triangle->vertices[1]];
	const BVector* vertexC = &navmesh->vertices[triangle->vertices[2]];
...
void projectPointOntoNavmesh(const BNavmesh* navmesh, const BVector* point,
							 const BTriangle** outTriangle, BVector* outPoint) {
	assert(navmesh->numTriangles >= 0);
...
static void projectPointOntoConnectedComponent(const BNavmesh* navmesh, const BVector* point,
...
static void getPortal(const BNavmesh* navmesh, const BTriangle* triangleA,
					  const BTriangle* triangleB, BEdge* outEdge) {
	...
	for (int i = 0; i < 3; i++) {
		...
		const int isShared = triangleB->vertices[0] == vertexIndex ||
							 triangleB->vertices[1] == vertexIndex ||
							 triangleB->vertices[2] == vertexIndex;
		const int isNextShared = triangleB->vertices[0] == nextVertexIndex ||
								 triangleB->vertices[1] == nextVertexIndex ||
								 triangleB->vertices[2] == nextVertexIndex;
...
static void funnelPath(const BNavmesh* navmesh, const BVector* start, const BVector* end,
					   const BAStarOutput* aStarOutput, BPath* outPath) {
	...
	for (int triangleIndex = 0; triangleIndex < numTriangles - 1; triangleIndex++) {
		...
		getPortal(navmesh, triangles[triangleIndex], triangles[triangleIndex + 1],
				  &portals[triangleIndex]);
	}
	...
	for (int portalIndex = 0; portalIndex < numPortals; portalIndex++) {
		...
		if (triangleArea2(&portalApex, &portalRight, right) <= 0) {
			if (vectorEquals(&portalApex, &portalRight) ||
				triangleArea2(&portalApex, &portalLeft, right) > 0) {
				// Tighten
				portalRight = *right;
		...
		if (triangleArea2(&portalApex, &portalLeft, left) >= 0) {
			if (vectorEquals(&portalApex, &portalLeft) ||
				triangleArea2(&portalApex, &portalRight, left) < 0) {
				// Tighten
				portalLeft = *left;
...
static void aStar(const BNavmesh* navmesh, const BTriangle* startTriangle,
				  const BTriangle* endTriangle, const BVector* destination, BAStarOutput* output) {
	assert(navmesh != NULL);
	...
	while (1) {
		...
		for (int neighborIndex = 0; neighborIndex < 3; neighborIndex++) {
			...
			int inOpenList = linkedListFind(&openList, isTriangle, (void*)neighborTriangle,
											&occurenceInOpenList);
			if (inOpenList) {
			...
			const int inClosedList =
				linkedListFind(&closedList, isTriangle, (void*)neighborTriangle, NULL);
...
void pathfinder(const BNavmesh* navmesh, const BVector* start, const BVector* end, BPath* outPath) {
	...
	const int isDestinationReachable =
		endTriangle != NULL &&
		endTriangle->connectedComponent == adjustedStartTriangle->connectedComponent;
	if (!isDestinationReachable) {
		projectPointOntoConnectedComponent(navmesh, end, adjustedStartTriangle->connectedComponent,
										   &adjustedEndTriangle, &adjustedEnd);
	} else {
		adjustedEnd = *end;
